...
with open("LUA/selsins_re.lua", "w") as f:
    f.write("")

with open("LUA/selsins_re.lua", "a") as f:
    ii = 0
    for y in range(len(selsins_result)):
        for x in range(len(selsins_result[y])):
            ...
            if selsins[y][x] == "1":
                folder = "p4"
                a = getglobalpos("p4", [-(x * ox + hs + 500 - 78 / 2) / 1000, -((y - 1) * oy + 110 + hs + 95 / 2 + YOFF) / 1000, 0])
                if x > 5:
                    folder = "p5"
                    a = getglobalpos("p5", [-((x - 6) * ox + hs + 25 - 78 / 2) / 1000, -((y - 1) * oy + 110 + hs + 95 / 2 + YOFF) / 1000, 0])
                if x > 16:
                    folder = "p6"
                    a = getglobalpos("p6", [-((x - 17) * ox + hs + 25 - 78 / 2) / 1000, -((y - 1) * oy + 110 + hs + 95 / 2 + YOFF) / 1000, 0])
                f.write(f"newmodel{ii} = workspace.prefabs.re_2l:clone()\n")
                f.write(f"newmodel{ii}:PivotTo(CFrame.new({a[0][0]}, {a[0][1]}, {a[0][2]}) * CFrame.fromEulerAngles(0, math.rad({-90 - a[1] + pan_rot}), 0))\n")
                f.write(f"newmodel{ii}.Parent = workspace.devices.sel_re2l\n")
                f.write(f"newmodel{ii}.Body1.Decal.Texture = \"rbxgameasset://Images/SEL_RE_{y}-{x}\"\n")
                ii += 1