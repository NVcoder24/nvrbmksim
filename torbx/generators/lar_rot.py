...
with open("LUA/lar_rot.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/lar_rot.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "larcorrectorrotary" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.lar_rot:clone()\n")
                    ...
                    f.write(f"newmodel{ii}.Parent = workspace.RBMKSYS.viur.larcorrector\n")
                    ii += 1
                else:
