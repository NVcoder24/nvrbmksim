...
with open("LUA/mk6.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/mk6.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "mk6" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.mk6:clone()\n")
                    ...
                    f.write(f"newmodel{ii}.Parent = workspace.devices.mk6\n")
                    ii += 1
                else:
