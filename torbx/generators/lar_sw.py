...
with open("LUA/lar_sw.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/lar_sw.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "larcorrectorsw" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.lar_sw:clone()\n")
