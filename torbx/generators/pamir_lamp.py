...
with open("LUA/pamir_lamp.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/pamir_lamp.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "pamirlamp" in j["name"] and "handle" not in j["name"]:
                a = getglobalpos_pult(i, j["pos"])
                f.write(f"newmodel{ii} = workspace.prefabs.pamir_lamp:clone()\n")
