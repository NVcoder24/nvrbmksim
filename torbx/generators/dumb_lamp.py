...
with open("LUA/dumb_lamp.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/dumb_lamp.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "dumb_lamp" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.dumb_lamp:clone()\n")
                    ...
                    f.write(f"newmodel{ii}.Parent = workspace.devices.dumb_lamp\n")
                    ii += 1
                else:
                    np = j["pos"]
                    np[2] = -np[2]
