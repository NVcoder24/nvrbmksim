...
with open("LUA/pamir_tumbler.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/pamir_tumbler.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "pamirtumbler" in j["name"] and "handle" not in j["name"]:
                a = getglobalpos_pult(i, j["pos"])
                f.write(f"newmodel{ii} = workspace.prefabs.pamir_tumbler:clone()\n")
