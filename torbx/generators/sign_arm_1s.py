...
with open("LUA/sign_arm_1s.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/sign_arm_1s.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "sign_arm_1s" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.sign_arm_1s:clone()\n")
                    ...
                    f.write(f"newmodel{ii}.Parent = workspace.devices.sign_arm_1s\n")
                    ii += 1
                else:
