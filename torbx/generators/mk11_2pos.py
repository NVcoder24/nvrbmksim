...
with open("LUA/mk11_2pos.lua", "w") as f:
    f.write("")

ii = 0
with open("LUA/mk11_2pos.lua", "a") as f:
    for i in data.keys():
        for j in data[i]:
            if "mk12" in j["name"] and "handle" not in j["name"]:
                if ispult(i):
                    a = getglobalpos_pult(i, j["pos"])
                    f.write(f"newmodel{ii} = workspace.prefabs.mk11:clone()\n")
                    ...
                    f.write(f"newmodel{ii}.Parent = workspace.devices.mk11\n")
                    ii += 1
                else:
